...
class Vendedores extends CI_Controller{
	...
	public function add(){
			...
			$this->form_validation->set_rules('vendedor_nome_completo','','trim|required|min_length[5]|max_length[200]');
			...
			$this->form_validation->set_rules('vendedor_email','','trim|required|valid_email|is_unique[vendedores.vendedor_email]');
			...
			if ($this->form_validation->run()) {
				...
				$this->session->set_flashdata('sucesso', 'vendedor salvo com sucesso');
				redirect('vendedores');
	...
	public function edit($vendedor_id = NULL){
		...
		if (!$vendedor_id || !$this->core_model->get_by_id('vendedores', array('vendedor_id' => $vendedor_id))) {
			...
			$this->form_validation->set_rules('vendedor_rg','','trim|max_length[20]|callback_check_rg');
			$this->form_validation->set_rules('vendedor_email','','trim|required|valid_email|max_length[50]|callback_check_email');
			$this->form_validation->set_rules('vendedor_telefone','','trim|required|max_length[15]|callback_check_telefone');
	...
	public function check_rg($vendedor_ie){
			$vendedor_id = $this->input->post('vendedor_id');

			if($this->core_model->get_by_id('vendedores', array('vendedor_rg' => $vendedor_ie, 'vendedor_id != ' => $vendedor_id))){

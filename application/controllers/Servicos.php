...
class Servicos extends CI_Controller{
	...
	public function add(){
			...
			$this->form_validation->set_rules('servico_nome','','trim|required|min_length[10]|max_length[20]');
			...
			if ($this->form_validation->run()) {
				...
				$this->session->set_flashdata('sucesso', 'servico salvo com sucesso');
				redirect('servicos');
	...
	public function edit($servico_id = NULL){
		...
		if (!$servico_id || !$this->core_model->get_by_id('servicos', array('servico_id' => $servico_id))) {
			...
			$this->form_validation->set_rules('servico_nome','','trim|required|min_length[10]|max_length[20]|callback_check_nome_servico');
			$this->form_validation->set_rules('servico_preco','','trim|required');
			$this->form_validation->set_rules('servico_descricao','','trim|max_length[500]');
	...
	public function del($servico_id = NULL){
			...
			if (!$servico_id || !$this->core_model->get_by_id('servicos', array('servico_id' => $servico_id))) {
				...
				$this->session->set_flashdata('sucesso', 'servico excluido com sucesso');
				redirect('servicos');
			}
	...
	public function check_nome_servico($servico_nome){
		...
		if($this->core_model->get_by_id('servicos', array('servico_nome' => $servico_nome, 'servico_id != ' => $servico_id))){
			$this->form_validation->set_message('check_nome_servico', 'Este nome já existe');
			return FALSE;
		} else {
			return TRUE;
		}
